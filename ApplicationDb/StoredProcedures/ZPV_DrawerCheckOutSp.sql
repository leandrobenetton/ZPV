/****** Object:  StoredProcedure [dbo].[ZPV_DrawerCheckOutSp]    Script Date: 16/01/2015 03:13:23 p.m. ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ZPV_DrawerCheckOutSp]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[ZPV_DrawerCheckOutSp]
GO
...
CREATE PROCEDURE [dbo].[ZPV_DrawerCheckOutSp] (
	@Drawer			varchar(15),
	@CheckOutUser	UsernameType,
	@CheckOutDate	BankCodeType,
...
BEGIN TRY
	...
	BEGIN
		IF EXISTS(SELECT * FROM zpv_drawer WHERE drawer = @Drawer AND check_in = 1 AND check_in_user = @CheckOutUser)
		BEGIN
			select 
			...
			BEGIN
				SET @Infobar = 'La Caja tiene Valores pendientes de cierre, no se puede realizar el CheckOut'		
			END
			ELSE
			BEGIN
				select 
				...
				begin
					...
					SET @Infobar = 'Caja Cerrada sin Diferencias'
				end
				else
				begin
					...
					SET @Infobar = 'Caja Cerrada con Diferencias'
				end
			END
		END
		ELSE
		BEGIN
			SET @Infobar = 'La Caja no está abierta o se encuentra abierta por otro usuario'
		END
	END
END TRY
BEGIN CATCH
