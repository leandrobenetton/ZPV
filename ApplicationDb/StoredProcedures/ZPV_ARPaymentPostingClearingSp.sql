/****** Object:  StoredProcedure [dbo].[ZPV_ARPaymentPostingClearingSp]    Script Date: 16/01/2015 03:12:10 p.m. ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ZPV_ARPaymentPostingClearingSp]') AND type in (N'P', N'PC'))	
DROP PROCEDURE [dbo].[ZPV_ARPaymentPostingClearingSp]
GO
...
CREATE PROCEDURE [dbo].[ZPV_ARPaymentPostingClearingSp] (
...
BEGIN
	...
	WHERE arpmt.bank_code = @PBankCode
	   ...
	   AND ((arpmt.type = 'W' AND arpmt.deposit_date IS NOT NULL AND arpmt.deposit_date <= dbo.GetSiteDate(GETDATE()))OR
